...
void goFORWARD(int distance) {
  ...
  if (!(rfwdIrVal.distance() < 130 || lfwdIrVal.distance() < 130 || mfwdIrVal.distance() < 120)) {
    ...
    if (distance <= 1500) {
      ...
      while (mCounter[0] < distance && mCounter[1] < distance) {
        if ((irFrontReadings[0] < (breakDist + 10)) || (irFrontReadings[1] < breakDist) || (irFrontReadings[2] < (breakDist + 10))) {
          break;
        }
        scanFORWARD(&irFrontReadings[0]);
        if (millis() - lastTime > 100) {
...
void goRIGHT(int angle) {
  ...
  while (mCounter[0] < angle - turnRightTicks - 200 && mCounter[1] < angle - turnRightTicks - 200) {
    if (millis() - lastTime > 100) {
      //      PIDControl(&setSpdR, &setSpdL, 150, 0, 15, 1);
      PIDControl(&setSpdR, &setSpdL, 20, 0, 40, 1);
      lastTime = millis();
      setSpdR = setSpdR + 1;
      setSpdL = setSpdL + 1;
...
void fwdCorrection() {
  ...
  if (fwdCounter == 3) {
    if (pullDist > 0) {
      while (mCounter[0] < abs(pullDist)) {       //Right motor pull back
        md.setM1Speed(-350);
      }
    }
